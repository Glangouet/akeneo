...
ID_RSA ?= ~/.ssh/id_rsa
DOCKER_NETWORK ?= meup
DOCKER_LOCAL_HOSTNAME ?= akeneo.local.1001pharmacies.com
DIR := ${CURDIR}
...
bootstrap-git:
	if ! git config remote.upstream.url > /dev/null ; \
		then git remote add upstream git@github.com:akeneo/pim-community-standard.git; \
	fi
...
import-database:
ifdef DUMP
	docker-compose exec akeneo sh -c 'mysql -h meup-mysql -u root -proot meup2 < $(DUMP)'
else
...
docker-connect:
	docker-compose exec akeneo /bin/bash
...
install-deps: install-composer create-db install-pim

## Install composer
install-composer: docker-ssh-agent
	docker-compose exec akeneo sh -c 'composer self-update && composer create-project --prefer-dist akeneo/pim-community-standard ./pim-project "1.7.*@stable" || true'
	docker-compose exec akeneo sh -c 'composer install --optimize-autoloader --prefer-dist'

create-db:
	docker exec meup-mysql sh -c "mysql -u root -proot akeneo_pim -e 'use akeneo_pim' >/dev/null 2>&1 || mysql -u root -proot mysql -e 'create database akeneo_pim character set utf8 collate utf8_unicode_ci;'"
	docker exec meup-mysql bash -c "mysql -u akeneo_pim -pakeneo_pim akeneo_pim -e 'use akeneo_pim' >/dev/null 2>&1 || mysql -u root -proot mysql -e \"grant all privileges on akeneo_pim.* to 'akeneo_pim'@'%' identified by 'akeneo_pim'; flush privileges;\""

install-pim: docker-ssh-agent
	docker-compose exec akeneo sh -c 'php app/console pim:install'

## Fix variable folders
fix: fix-cache fix-permissions

fix-permissions:
	docker-compose exec akeneo sh -c 'chmod -R 0777 app/cache'
	docker-compose exec akeneo sh -c 'chmod -R 0777 app/logs'

fix-cache:
	docker-compose exec akeneo sh -c 'php app/console cache:clear'
